-- 020_fee_tax_attribution_active_ledger_reference_hardening.sql
-- Sentinax Private Personal Investment Decision Engine
-- Phase 14G.1: Active Ledger-Reference & Reversal-Race Hardening
--
-- Migration Purpose:
--   1. Hardens public.validate_fee_tax_attribution_event_integrity() on
--      public.portfolio_fee_tax_attribution_events to:
--        - Lock both charge AND target ledger transactions with FOR UPDATE (deterministic order).
--        - Enforce that neither the referenced charge nor target was reversed at or before attribution recorded_at.
--        - Preserve all Phase 14F/G domain, capacity, causality, and monotonic-history invariants.
--   2. Adds narrow synchronization trigger on public.portfolio_transactions
--      (public.lock_portfolio_transaction_reversal_target) to lock the reversed transaction row
--      with FOR UPDATE on ledger REVERSAL insertion, closing the concurrency race window between
--      attribution insertion and concurrent ledger reversal.

-- ============================================================================
-- 1. Ledger REVERSAL Synchronization Trigger Function
-- ============================================================================
-- Narrow lock-only function. Does not duplicate or modify closed ledger validation.
