...
class BISTKMTPBulletinParser:
    ...
    def parse_xlsx_bytes(
        ...
        def read_sheet_data(sheet_num: int) -> Dict[int, Dict[str, str]]:
            # Try worksheet{num}.xml and sheet{num}.xml
            targets = [
        ...
        # 3. Parse Sheet 'Fiyatlar' (Summary Benchmark Rates)
        ...
        if fiyatlar_idx:
            ...
            raw_observations.extend(
                self._parse_fiyatlar_sheet(
                    rows=fiyatlar_rows,
                    effective_date=eff_date or date.today(),
                    snapshot_id=snapshot_id,
        ...
        # 4. Parse Sheet 'Seri İstatistikleri' (Granular Transaction Series)
        ...
        # 5. Quarantine conflicting duplicate observations deterministically
    ...
    def _parse_seri_istatistikleri_sheet(
        ...
        for r_num in sorted(rows.keys()):
            ...
            # Parse purity (Ayar)
            purity = parse_kmtp_decimal(cells.get("I"))

            # Parse settlement/value date
            settlement_val = cells.get("K", "").strip()
            ...
            if aof is not None:
                obs_aof = PreciousMetalMarketObservation(
                    ...
                    purity=purity,
                    settlement_term=settlement_val or "T+0",
                    volume=volume,
                    turnover=turnover,
