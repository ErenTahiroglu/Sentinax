"""
backend/engine/private/providers/eurostat_sdmx.py
===================================================
Eurostat Dissemination SDMX 2.1 / SDMX-CSV Data Provider.

Official Specification:
    - Base URL: https://ec.europa.eu/eurostat/api/dissemination/
    - Protocol: SDMX 2.1 REST Dissemination Service
    - Entry Point: /sdmx/2.1/data/{flowRef}/{key}
    - Format: SDMX-CSV (format=SDMX-CSV)
    - Authentication: Open public service (No API key required).

Point-in-Time & Semantic Invariants:
    - Missing observations (":", "", ".") are strictly parsed as None; 0.0 is preserved.
    ...
    - Historical SYSTEM_AS_OF and SOURCE_AS_OF are rejected (Eurostat dissemination API does not support PIT vintage reconstruction).
    - Dataset update metadata is not fabricated as publication timestamp (`published_at = None`).
"""
...
class EurostatSDMXProvider(DataProviderContract):
    """
    Adapter for Eurostat Official Dissemination SDMX 2.1 REST API.
    """
    provider_name: str = "EUROSTAT"
    ...
    async def fetch(self, context: FetchContext) -> ProviderResponse:
        """
        Fetches macroeconomic observations from Eurostat.
        """
        ...
        if not series_code:
            return ProviderResponse(
                ...
                status=DataStatus.UNAVAILABLE,
                raw=None,
                warnings=["No valid Eurostat series code or dataset specified."],
                canonical_instrument_id=context.canonical_instrument_id,
                provider_symbol=context.provider_symbol,
            )
        ...
        if context.is_historical:
            if context.as_of_mode == AsOfMode.SYSTEM_AS_OF:
                ...
                return ProviderResponse(
                    ...
                    status=DataStatus.UNAVAILABLE,
                    raw=None,
                    warnings=["Eurostat historical source vintage unavailable; local PIT storage required."],
                    canonical_instrument_id=context.canonical_instrument_id,
                    provider_symbol=context.provider_symbol,
                )
            else:
        ...
        # 3. Parse dataset flowRef and series key (Format: FLOW_REF/KEY)
        ...
        if not flow_ref or not key:
            return ProviderResponse(
                ...
                status=DataStatus.UNAVAILABLE,
                raw=None,
                warnings=[f"Invalid Eurostat series code format: '{series_code}'. Expected 'DATASET/KEY'."],
                canonical_instrument_id=context.canonical_instrument_id,
                provider_symbol=context.provider_symbol,
            )
        ...
        endpoint_url = f"{self.base_url}sdmx/2.1/data/{flow_ref}/{key}"
        ...
        try:
            ...
            raise ProviderTimeoutError(f"Eurostat API request timed out: {e}", provider_name=self.provider_name)
        except httpx.NetworkError as e:
            raise ProviderServerError(f"Eurostat API network error: {e}", provider_name=self.provider_name)

        if resp.status_code in (400, 404):
            raise ProviderInvalidSymbolError(f"Eurostat series '{flow_ref}/{key}' not found or parameters invalid.", provider_name=self.provider_name)
        if resp.status_code in (401, 403):
            raise ProviderAuthenticationError("Eurostat API access forbidden.", provider_name=self.provider_name)
        if resp.status_code == 429:
            raise ProviderRateLimitError("Eurostat API rate limit exceeded.", provider_name=self.provider_name)
        if resp.status_code >= 500:
            raise ProviderServerError(f"Eurostat server error: HTTP {resp.status_code}", status_code=resp.status_code, provider_name=self.provider_name)
        if resp.status_code != 200:
            return ProviderResponse(
                ...
                status=DataStatus.UNAVAILABLE,
                raw=None,
                warnings=[f"Eurostat returned HTTP {resp.status_code}"],
                canonical_instrument_id=context.canonical_instrument_id,
                provider_symbol=context.provider_symbol,
            )
        ...
        if not rows:
            return ProviderResponse(
                ...
                warnings=["Eurostat returned 0 observations for requested query."],
                canonical_instrument_id=context.canonical_instrument_id,
                provider_symbol=context.provider_symbol,
            )
        ...
        if eff_date is None:
            return ProviderResponse(
                ...
                warnings=[f"Eurostat observation date was unparseable: '{time_period_str}'."],
                canonical_instrument_id=context.canonical_instrument_id,
                provider_symbol=context.provider_symbol,
            )
        ...
        source_meta: Dict[str, Any] = {
            "delivery_provider": "Eurostat Dissemination API",
            "source_role": "OFFICIAL_STATISTICAL_AUTHORITY",
            "origin_source": canonical_def.origin_source if canonical_def else "Eurostat (Statistical Office of the European Union)",
            "release_name": canonical_def.release_name if canonical_def else "Eurostat Statistical Release",
            "dataset_code": flow_ref,
            "series_key": key,
            "geo": latest_row.get("geo") or latest_row.get("GEO"),
        ...
        return ProviderResponse(
            ...
            status=status,
            raw={"csv_text": csv_text, "rows": rows, "latest": latest_row},
            warnings=[] if status == DataStatus.COMPLETE else [f"Eurostat observation value is missing (raw: '{raw_val}')."],
            canonical_instrument_id=context.canonical_instrument_id,
            provider_symbol=context.provider_symbol,
    ...
    def provenance(self, response: ProviderResponse) -> ProviderProvenance:
        ...
        meta["delivery_provider"] = "Eurostat Dissemination API"
        meta["source_role"] = "OFFICIAL_STATISTICAL_AUTHORITY"
        ...
        return ProviderProvenance(
            ...
            endpoint=f"{self.base_url}sdmx/2.1/data",
    ...
    def _parse_decimal(val_str: Any) -> Optional[float]:
        ...
        # Eurostat uses ":" or "." for missing/unrecorded observations
